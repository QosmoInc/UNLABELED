...
class PatchTrainer(object):
    def __init__(self, mode):
        ...
        self.prob_extractor = MaxProbExtractor(15, 80, self.config).to(device) # 15 is cat
    ...
    def train(self):
        ...
        orig_img = self.read_image('imgs/pattern検証0721-02.png').to(device)
        orig_img_style = self.read_image('imgs/pattern検証0721-02.png').to(device)
        ...
        for epoch in range(1, n_epochs):
            ...
            for i_batch, (img_batch, lab_batch) in tqdm(enumerate(train_loader), desc=f'Running epoch {epoch}',
                ...
                with autograd.detect_anomaly():
                    ...
                    adaIN_loss = self.adaIN_style_loss(adv_patch.unsqueeze(0), orig_img_style.unsqueeze(0).to(device)) * 4.5
                    c_loss = self.content_loss(adv_patch, orig_img) * 0.0
